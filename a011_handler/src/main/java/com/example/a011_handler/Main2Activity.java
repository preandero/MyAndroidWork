 ...
 *
 *  사용 방법:
 *      ▫ 'Handler 를 생성'한 스레드만이 다른 작업스레드가 전송하는 'Message' 나 'Runnable 객체' 를 수신하는 기능을 할 수 있다.
        ...
        public void run() {
            while(true){
                ...
                handler2.post(new Runnable() {
                    @Override
                    public void run() { // Runnable 의 run() 에서 메인 UI 접
...
class BackThread3 extends Thread{
    ...
    public void run() {
        ...
        while(true){
            ...
            handler.sendMessage(msg); // 메인스레드의 핸들러에 메세지 보내
            try {
                Thread.sleep(1000);
            } catch (InterruptedException e) {
                e.printStackTrace();
